...
class Latencies(object):
    """
    An extension that measure throughput and latencies
    """
    ...
    def _spider_opened(self, spider):
        self.task = task.LoopingCall(self._log, spider)  # 以interval为周期循环调用方法
    ...
    def _log(self, spider):
        irate = float(self.items) / self.interval  # 吞吐量
        latency = self.latency / self.items if self.items else 0  # 平均延时
        proc_latency = self.proc_latency / self.items if self.items else 0  # 平均处理延时
